...
if sys == 8 % sys == T8 Thomas mass parameter
...
x_CJL = LP(:,1) ;
y_CJL = LP(:,2) ;
CJ_L = x_CJL.^2 + y_CJL.^2 + 2*(1-mu)./sqrt((x_CJL + mu).^2 + y_CJL.^2) + 2*mu./sqrt((x_CJL - (1-mu)).^2 + y_CJL.^2) ;
...
y20 = -0.0780 ;
z20 = 0.0040 ;
beta_deg = round(rad2deg(0.4014257)/0.5)*0.5 ;
beta = deg2rad(beta_deg) ;

r20 = sqrt(x20^2 + y20^2 + z20^2) ;
v20 = sqrt(2*mu/r20) ;
...
y0B = y20 ;
z0B = z20 ;
r10 = sqrt(x10^2 + y10^2 + z10^2) ;
gamma = atan2(y20, -x20) ;
sigma = asin( (2*(1-mu)/r10 + x0B^2 - x20^2 - CJ) / (2*sqrt(x20^2+y20^2)*v20*cos(beta)) ) ; % compact formulation
% sigma = asin( (2*(1-mu)/r10 + 2*(1-mu)*x0B - (1-mu)^2 - CJ) / (2*sqrt(x20^2+y20^2)*v20*cos(beta)) ) ; % alternative formulation
% % % sigma = asin( (2*(1-mu)/r10 + x0B^2 - x20^2 - CJ) / (2*sqrt(x20^2+y20^2)*v20) ) ; % wrong angle for V17_zdot: AutonomousSearchAlgorithm_V17_zdot.m
...
CJ_history = -(xdot.^2 + ydot.^2 + zdot.^2) + x.^2 + y.^2 + 2*(1-mu)./r1 + 2*mu./r2 ;
disp(CJ_history(1))
